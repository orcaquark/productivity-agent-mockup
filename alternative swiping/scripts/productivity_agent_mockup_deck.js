// ---------------- card data ----------------
  ...
  //
  // The deck has its own local progress so that reopening this particular
  // mockup restores the exact card position. SharedState stores the
  // product-level consequences of decisions so other mockups can see them.
  //
  function getSharedTasks(){
    return SharedState.get("tasks", {
      expense: false,
      dentist: false
    });
  }

  function updateSharedTask(id, completed){
    if(id !== "missExpense" && id !== "missDentist") return;

    var taskId = id === "missExpense" ? "expense" : "dentist";
    SharedState.update("tasks." + taskId, completed);
  }

  function getSharedDeckState(){
    return SharedState.get("swiping", {
      resolved: [],
      skipped: []
    });
  }

  function saveSharedDeckState(){
    SharedState.update("swiping", {
      resolved: Array.from(resolvedIds),
      skipped: skipped.slice()
    });
  }

  function restoreSharedDeckState(){
    var shared = getSharedDeckState();

    if(Array.isArray(shared.resolved)){
      shared.resolved.forEach(function(id){
        resolvedIds.add(id);
      });
    }

    if(Array.isArray(shared.skipped)){
      skipped = shared.skipped.slice();
    }
  }
  ...
  function buildCardEl(id, role){
    ...
    if(role === "top"){
      ...
      rs.id = "stamp-right";
      rs.textContent = stampText(id, "right");

      var ls = document.createElement("div");
      ls.className = "stamp left";
      ls.id = "stamp-left";
      ls.textContent = stampText(id, "left");
  ...
  function renderColdStart(){
    ...
    stackArea.querySelectorAll(".stack-card:not([data-exit])").forEach(function(e){
      e.remove();
    });
  ...
  function updateProgress(){
    ...
    document.getElementById("progress-count").textContent =
      queue.length
        ? (Math.min(done + 1, TOTAL) + " of " + TOTAL + " today")
        : (TOTAL + " of " + TOTAL + " today");
  ...
  function updateActionRow(){
    ...
    if(mode === "new"){
      row.innerHTML =
        '<button class="action-btn primary" style="flex:1" onclick="openNavModal(\'🤝\',\'This would open Friends\',\'Deep-links to the Friends tab so you can find people to follow. Nothing about your tasks or rings is shared until you choose to send kudos.\')">Add friends</button>';
      return;
    }
    ...
    if(d.kind === "info"){
      row.innerHTML =
        '<button class="action-btn primary" style="flex:1" onclick="resolveTop(\'right\')">Continue →</button>';
    } else {
  ...
  function applyResolution(id, dir){
    ...
    // Persist consequences that matter to the product.
    if(id === "missExpense" || id === "missDentist"){
      updateSharedTask(id, dir === "right");
    }
    ...
    if(d.kind === "decision"){
      if(dir === "left"){
        ...
        showToast(d.toastLeft, true);
        announce(d.toastLeft);
      } else {
        showToast(d.toastRight, true);
        announce(d.toastRight);
      }

      // Store the decision itself so other mockups can respond to it.
      SharedState.update("swiping.lastDecision", {
        id: id,
        direction: dir,
        timestamp: Date.now()
      });

      MockSync.broadcast("card-resolved", {
        id: id,
        dir: dir
      });
    } else {
  ...
  function undoLast(){
    ...
    var id = lastAction.id;
    var dir = lastAction.dir;

    queue.unshift(id);
    resolvedIds.delete(id);

    if(dir === "left"){
      skipped = skipped.filter(function(s){
        return s.id !== id;
      });
    }

    if(id === "missExpense" || id === "missDentist"){
      updateSharedTask(id, false);
    }
    ...
    SharedState.update("swiping.lastDecision", null);
  ...
  function commitSwipe(el, id, dir, yOffset){
    ...
    var stampEl = document.getElementById(
      dir === "right" ? "stamp-right" : "stamp-left"
    );
    ...
    requestAnimationFrame(function(){
      ...
      el.style.transform =
        "translate(" + outX + "px, " + yOffset + "px) rotate(" +
        (dir === "right" ? 26 : -26) + "deg)";
  ...
  function onPointerMove(e){
    ...
    dragEl.style.transform =
      "translate(" + dx + "px, " + (dy * 0.35) + "px) rotate(" +
      (dx / 16) + "deg)";

    var threshold = 110;

    var rightOp = Math.max(
      0,
      Math.min(1, dx / threshold)
    );
    ...
    var sr = document.getElementById("stamp-right");
    var sl = document.getElementById("stamp-left");

    if(sr) sr.style.opacity = rightOp;
    if(sl) sl.style.opacity = leftOp;

    dragEl.dataset.dx = dx;
    dragEl.dataset.dy = dy;
  }
  ...
  function toggleTrend(){
    ...
    document.getElementById("trend-week-view").style.display =
      trendMonthOpen ? "none" : "block";

    document.getElementById("trend-month-view").style.display =
      trendMonthOpen ? "block" : "none";

    document.getElementById("trend-big").innerHTML =
      trendMonthOpen ? "69<small>%</small>" : "82<small>%</small>";

    document.getElementById("trend-delta").textContent =
      trendMonthOpen
        ? "trending up over 4 weeks"
    ...
    document.getElementById("trend-toggle-btn").textContent =
      trendMonthOpen
        ? "← Back to this week"
        : "See full month →";
  }
  ...
  function openRingModal(){
    openNavModal(
      "🔗",
      "This would open Rings",
      "In the real app, this deep-links to the Rings tab scoped to Gym — it doesn't close the ring from here. Closing a ring likely involves more than one tap (duration, reps, streak), so this mockup shows the intended destination instead of faking that interaction."
    );
  }
        ...
        return (
          '<div class="sheet-item">' +
            '<div class="sheet-item-text">' +
              '<div class="t">' + s.reason + '</div>' +
              '<div class="r">' + cardDefs[s.id].label + '</div>' +
            '</div>' +
  ...
  function setState(s){
    ...
    document.getElementById("pill-active").classList.toggle(
      "active",
      s === "active"
    );

    document.getElementById("pill-new").classList.toggle(
      "active",
      s === "new"
    );
    ...
    // "New user" is an intentional reset of the demo deck.
    // Keep shared product data intact unless explicitly resetting
    // the entire product elsewhere.
    if(s === "active"){
      restoreSharedDeckState();

      // Don't re-add cards that the shared state says were already resolved.
  ...
  (function initDeck(){
    ...
    if(saved){
      ...
      resolvedIds = new Set(
        Array.isArray(saved.resolvedIds)
          ? saved.resolvedIds
          : []
      );
      ...
      // SharedState is authoritative for product-level state.
      restoreSharedDeckState();

      // Make sure cards already resolved through another interface
      // don't reappear in the swiping deck.
      if(mode === "active"){
        queue = queue.filter(function(id){
          return !resolvedIds.has(id);
        });
      ...
      var onboardingComplete = MockState.load(
        "onboarding-complete",
        false
      );
  ...
  // ---------------- live shared-state synchronization ----------------
  SharedState.listen(function(state){
    if(mode !== "active") return;

    var sharedResolved =
      state &&
      state.swiping &&
      Array.isArray(state.swiping.resolved)
        ? state.swiping.resolved
        : [];

    var changed = false;

    sharedResolved.forEach(function(id){
